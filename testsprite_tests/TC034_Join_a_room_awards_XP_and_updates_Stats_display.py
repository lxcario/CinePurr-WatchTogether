...
async def run_test():
    ...
    try:
        ...
        # -> Retry signing in by clicking the Sign In button again to see if login succeeds (second attempt). If it fails again, report the issue per instructions.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[3]/main/div[6]/form/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        ...
        frame = context.pages[-1]
        assert '/room' in await frame.evaluate("() => window.location.href")
        assert await frame.locator("xpath=//*[contains(., 'XP')]").nth(0).is_visible(), "Expected 'XP' to be visible"
        assert await frame.locator("xpath=//*[contains(., 'Level')]").nth(0).is_visible(), "Expected 'Level' to be visible"
