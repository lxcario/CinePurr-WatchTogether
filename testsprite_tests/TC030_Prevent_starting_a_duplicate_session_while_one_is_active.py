...
async def run_test():
    ...
    try:
        ...
        # -> Enter username and password into the Sign In form and submit (click Sign In).
        ...
        # -> Dismiss the welcome/tutorial modal so the top navigation (LOGIN) is accessible. Click the modal 'Skip' button (interactive element index 1274).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/main/div[7]/div/div[3]/div/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Click the 'LOGIN' link in the top navigation to open the login page so credentials can be entered (use interactive element index 895).
        frame = context.pages[-1]
        # Click element
        ...
        assert await frame.locator("xpath=//*[contains(., 'already active')]").nth(0).is_visible(), "Expected 'already active' to be visible"
