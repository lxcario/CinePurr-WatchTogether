...
async def run_test():
    ...
    try:
        ...
        # -> Close the welcome modal (click 'Skip'), then click the public room 'Resque's Room' to join it.
        ...
        # -> Sign in using provided credentials (username: Lucario, password: ***REMOVED***) and wait for the app to load the homepage/room list.
        frame = context.pages[-1]
        # Input text
        ...
        frame = context.pages[-1]
        assert await frame.locator("xpath=//*[contains(., 'Invalid YouTube URL')]").nth(0).is_visible(), "A URL validation error should be visible and the video should not be added to the queue after submitting an invalid YouTube URL"
